...
import numpy as np
from secrets import config_fir
...
def index():
    ...
    req_mengajar_l = requests.get('http://localhost/top-presence/api/v1/ruangan/find-mengajar')
...
def buatlatih():
    if request.method == 'POST':
        ...
        # select id kelas
        req_kelas = requests.get('http://localhost/top-presence/api/v1/ruangan/find-kelas?kelas='+str(kelas))
        ...
        # select id matkul
        req_matkul = requests.get('http://localhost/top-presence/api/v1/ruangan/find-matkul?matkul='+str(matkul))
        ...
        req_mengajar = requests.get('http://localhost/top-presence/api/v1/ruangan/find-dosen?dosen='+str(id_kelas)+'&matkul='+str(id_matkul))
        ...
        req_mengambil = requests.get('http://localhost/top-presence/api/v1/ruangan/find-mhs?mhs='+str(id_mengajar))
        ...
        req_pertemuan = requests.get('http://localhost/top-presence/api/v1/ruangan/find-presensi?presensi='+str(id_mengajar))
        ...
        for y in master_req_mengambil:
            req_mahasiswa = requests.get('http://localhost/top-presence/api/v1/ruangan/find-mahasiswa?mahasiswa='+str(y['nim']))
...
def prediksi(kelas, presensi, matkul):
    ...
    for image_file in os.listdir(path_data_uji_presensi):
        ...
        for nama, (top, right, bottom, left), dist in prediksi:
            ...
            # simpan di firebase
            req_getnim = requests.get('http://localhost/top-presence/api/v1/ruangan/find-nim-mahasiswa?mahasiswa='+str(nama)) # NAMA HARUS BENAR
            ...
            if (len(json_req_getnim['master']) == 0): # cek jika kosong
                ...
                req_getnim = requests.get('http://localhost/top-presence/api/v1/ruangan/find-nim-mahasiswa?mahasiswa='+str(revisi)) # NAMA HARUS BENAR
                json_req_getnim = req_getnim.json()
    ...
    # combine hasil prediksi wajah kelas
    get_url = ""
    if len(file_arr) == 2:
    ...
    req_presensi = requests.post('http://localhost/top-presence/api/v1/presensi-detail/url-presensi-wajah', data=data_presensi)
...
def isi_presensi(kelas, presensi, matkul):
    ...
    for x in data:
        ...
        req_cek_presensi = requests.get('http://localhost/top-presence/api/v1/ruangan/find-mhs-detail-presensi?id_p='+str(presensi)+'&mhs='+str(x['nim']))
        ...
        if master_req_cek_presensi[0]['status'] == 'Tidak Hadir':
            ...
            req_presensi = requests.post('http://localhost/top-presence/api/v1/presensi-detail/terima-presensi-wajah', data=data_presensi)
...
def single_auth(nim, presensi):
    ...
    req_mahasiswa = requests.get('http://localhost/top-presence/api/v1/ruangan/find-mahasiswa?mahasiswa='+str(nim))
    json_req_mahasiswa = req_mahasiswa.json()
    master_req_mahasiswa = json_req_mahasiswa['master']
...
@app.route('/konfirmasi_auth/<nim>/<presensi>', methods=['GET'])
def konfirmasi_auth(nim, presensi):
    response = []
    ...
    data = []
    with open(path_hasil_uji_presensi+"/hasil.txt") as hasil:
    ...
    req_cek_presensi = requests.get('http://localhost/top-presence/api/v1/ruangan/find-mhs-detail-presensi?id_p='+str(presensi)+'&mhs='+str(nim))
    json_req_cek_presensi = req_cek_presensi.json()
    master_req_cek_presensi = json_req_cek_presensi['master']
    if master_req_cek_presensi[0]['status'] == 'Tidak Hadir':
        req_presensi = requests.post('http://localhost/top-presence/api/v1/presensi-detail/terima-presensi-wajah', data=data_presensi)
        ...
        if response_req['code'] == '200':
            ...
            if os.path.exists(path_hasil_uji_presensi+"/hasil.txt"):
                os.remove(path_hasil_uji_presensi+"/hasil.txt")
            database.child(f"uploads/single/{presensi}").child(nim).remove()
        else:
            print("[STATUS] Failed")
...
# route single create data latih
@app.route('/single_create/<nim>', methods=['GET'])
def single_create(nim):
    response = []
...
@app.route('/single_detect/<nim>', methods=['GET'])
def single_detect(nim):
    
    return jsonify("testing")

# Run Server
if __name__ == '__main__':
    # scheduler.add_job(id='Buat&Latih Data', func = buat_latih_data, trigger='cron', day_of_week='0-6', hour = 18) # tz UTC
    # scheduler.start()
